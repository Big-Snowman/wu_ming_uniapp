		...
		<view class="card" v-for="(item, index) in writingData" :key=item.id>
			...
			<view class="text-info">
				...
				<view class="text-info-bottom">
					...
					<view class="card-actions-item" @click="removeWriting(item, index)">
						<uni-icons type="trash" size="22" color="#000"></uni-icons>
	...
	export default {
		...
		methods: {
			...
			async getWriting() {
				this.getUserInfo()
				const { data } = await this.$myRequest({
					url: `/writing/getWritingByUserId?page=0&userId=${this.userInfo.id}`
				})
			...
			async loadMore() {
				...
				const { data } = await this.$myRequest({
					url: `/writing/getWritingByUserId?page=${this.page}&userId=${this.userInfo.id}`
				})
			...
			async actionsLikeClick(writing, index){
				...
				this.userInfo.receiveLikeNum += this.myLike[index] ? 1 : -1
				// 缓存
				...
				uni.showModal({
					...
					success: async function (res) {
						if (res.confirm) {
							...
							that.userInfo.myPublishNum --
							that.userInfo.receiveCommentNum += data.data.receiveCommentNum
							that.userInfo.receiveLikeNumreceiveLikeNum += data.data.receiveLikeNum
							that.writingData.splice(index,1)
							uni.showToast({
								title: '删除成功',
								icon: 'success'
							})
							...
							uni.$emit('removeWriting', item.id)
						} else if (res.cancel) {
						}
					}
				});
