<template>
	<view>
		<!-- 发表按钮 -->
		<view class="addnew">
			<uni-icons type="plus-filled" size="55" @click="toWritingPage"></uni-icons>
		</view>
		<!-- 回到顶部 -->
		<view class="toTop">
			<!-- <uni-icons type="arrow-up" size="35" @click="toTop" :style="'display: none'"></uni-icons> -->
...
<script>
	export default {
		...
		onLoad() {
			this.changeLikeList = uni.getStorageSync('changeLikeList') ? uni.getStorageSync('changeLikeList') : []
			this.init()
			// this.getWriting()
			...
			// 更新被删除的writing
			uni.$on('removeWriting', data => {
				for (let i = 0; i < this.writingData.length; i++) {
					if(data === this.writingData[i].id) {
						this.writingData.splice(i, 1)
					}
				}
			})
		},
		onUnload() {
			...
			uni.setStorageSync('changeLikeList', this.changeLikeList)
		},
		onPullDownRefresh() {
			this.page = 0,
		...
		methods: {
			...
			async init() {
				...
				if (res.code && ['login:ok'].includes(res.errMsg)) {
					...
					this.getUserLikeList(data.openid)
				} else {
					//异常处理，再次发起请求或者抛出异常
					uni.showToast({
						title: '系统异常，请联系管理员!'
			...
			async getWriting() {
				const writingDataUrl = this.dataPattern === 'new' ? `/writing?page=0` : `/writing/getNewHot?page=0`
				const { data } = await this.$myRequest({
					url: writingDataUrl
				})
			...
			async loadMore() {
				...
				this.page++
				const writingDataUrl = this.dataPattern === 'new' ? `/writing?page=${this.page}` : `/writing/getNewHot?page=${this.page}`
				const { data } = await this.$myRequest({
					url: writingDataUrl
				})
